 ...
 *
 * This class is for under Android 8.0.0 (API 26)
 *
...
public class DateParser {
    ...
    /**
     * Get the Date object after parsing input
     *
     * @param input the input string with date and time information
     * @return a Date object indicate the date and time after parsing
     */
     ...
     * @return a Date object indicate the date and time after parsing
     */
    public static Date getDateTime(String input, int currentWeek) {
        ...
        Date parseDateTime = calendar.getTime();
    ...
    /**
     * Get the Date object after parsing input with allow day different
     *
     * @param input the input string with date and time information
     * @param currentWeek a int value indicate the week now
     ...
     * @return a Date object indicate the date and time after parsing
     */
    public static Date getDateTime(String input, int currentWeek, int allowDayDifferent) {
        Date resultDateTime = getDateTime(input, currentWeek);
        if (allowDayDifferent > 0) {
            Date currentDateTime = new Date();
            long dayDiff = Math.abs((int) ((resultDateTime.getTime() - currentDateTime.getTime()) / (1000 * 60 * 60 * 24)));
     ...
     * @param currentDateTime a Date object indicate the date and time in the parsing phase
     * @param currentWeek a int value indicate the week now
     * @return a Date object indicate the date and time after parsing date
     */
    private static Date parseDate(String[] dateStringAry, Date currentDateTime, int currentWeek) {
        ...
        for (String dateString : dateStringAry) {
            ...
            calendar.add(Calendar.DATE, weekDiff * 7);

            int dayOfWeekNow = calendar.get(Calendar.DAY_OF_WEEK) - 1;
            dayOfWeekNow = dayOfWeekNow == 0 ? 7 : dayOfWeekNow;
            int dayOfWeekNeed = parseDayOfWeek(dateString, calendar.getTime());
            int dayOfWeekDiff = dayOfWeekNeed - dayOfWeekNow;
            calendar.add(Calendar.DATE, dayOfWeekDiff);
        }
     ...
     * @param currentDateTime a Date object indicate the date and time in the parsing phase
     * @return a Date object indicate the date and time after parsing time
     */
    private static Date parseTime(String[] timeStringAry, Date currentDateTime) {
        ...
        int hourNeed = parseHour(timeString, calendar.getTime());
        int hourDiff = hourNeed - calendar.get(Calendar.HOUR_OF_DAY);
        calendar.add(Calendar.HOUR_OF_DAY, hourDiff);

        int minuteNeed = parseMinute(timeString, calendar.getTime());
        int minuteDiff = minuteNeed - calendar.get(Calendar.MINUTE);
     ...
     */
    private static int parseMonth(String dateString, Date currentDateTime) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(currentDateTime);
        if (Pattern.matches(".*((下个?月)|(一个?月后)).*", dateString)) {
            ...
            return calendar.get(Calendar.MONTH) + 1 + 2;
        } else if (Pattern.matches(".*三个?月后.*", dateString)) {
            return calendar.get(Calendar.MONTH) + 1 + 3;
        }
        ...
        int monthNeed = calendar.get(Calendar.MONTH) + 1;
     ...
     */
    private static int parseDay(String dateString, Date currentDateTime) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(currentDateTime);
        if (Pattern.matches(".*明[天日早晚].*", dateString)) {
            ...
            return calendar.get(Calendar.DATE) + 2;
        } else if (Pattern.matches(".*(((大后|后三)[天日])|(三[天日]后)).*", dateString)) {
            return calendar.get(Calendar.DATE) + 3;
        }
        ...
        int dayNeed = calendar.get(Calendar.DATE);
     ...
     */
    private static int parseDayOfWeek(String dateString, Date currentDateTime) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(currentDateTime);
        ...
        int dayOfWeekNeed = now;
        if (Pattern.matches(patternDayOfWeek, dateString)) {
            int dayOfWeekNow = dayOfWeekNeed;
     ...
     */
    private static int parseHour(String timeString, Date currentDateTime) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(currentDateTime);
        ...
        int hourNeed = calendar.get(Calendar.HOUR_OF_DAY);
     ...
     */
    private static int parseMinute(String timeString, Date currentDateTime) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(currentDateTime);
        ...
        int minuteNeed = calendar.get(Calendar.MINUTE);
